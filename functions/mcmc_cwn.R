mcmc_cwc <- function(
  ...
  do_ind = FALSE
    ...
    {
        lambda_adapt[[id]] <- 1
        par_cop_mcmc[[id]] <- rep(NA, 2)
        par_cop_mcmc[[id]][1] <- log(rho_mcmc[id] / (1 - rho_mcmc[id]))
        par_cop_mcmc[[id]][2] <- mu_mcmc[id]
        mean_adapt[[id]] <- par_cop_mcmc[[id]] + rnorm(2, 0, 0.001)
        sigma_adapt[[id]] <- diag(0.01, 2)
        chol_adapt[[id]] <- t(chol(lambda_adapt[[id]] * sigma_adapt[[id]] + diag(1e-8, 2)))
        alpha_par_cop[[id]] <- 0
    }
    ...
    for (imcmc in 1:sample_to_save)
    {
        for (jmcmc in 1:burn_thin)
        {
            ...
            ## missing
            if (there_are_na) {
                # error("missing non properly implemented for this model")
                x_s_prop <- x_s_mcmc
                x_c_prop <- x_c_mcmc
            ...
            if (1 == 2) {
                ...
                for (id in 1:d)
                {
                    ...
                    if (!is.finite(mh_ratio)) {
                        print("mh_ratio is NA/NaN/Inf")

                        mh_ratio <- -Inf
                    }
                    alpha_mh <- min(1, exp(mh_ratio))
                    if (!is.finite(alpha_mh)) {
                ...
                for (id in 1:d) {
                    par_cop_mcmc[[id]][1] <- qlogis(rho_mcmc[id])
                    par_cop_mcmc[[id]][2] <- mu_mcmc[id]
                }
            }
            # ! sample rpecision, mean and r
            x_s_prop <- x_s_mcmc
            x_c_prop <- x_c_mcmc
            theta_cop_prop <- theta_cop
            r_prop <- r_mcmc
            for (id in 1:d)
            {
                ...
                # ! devo cambiare
                par_cop_prop <- t(chol_adapt[[id]] %*% rnorm(2))[1, ] + par_cop_mcmc[[id]]

                # rho_mcmc_app <- log(rho_mcmc[id] / (1 - rho_mcmc[id]))
                # rho_prop_app <- rnorm(1, rho_mcmc_app, sd_rho[id])
                rho_prop <- plogis(par_cop_prop[1]) # exp(rho_prop_app) / (1 + exp(rho_prop_app))
                mu_prop <- par_cop_prop[2]
                # ! calcolo la nuova copula e osservazioni latenti
                theta_cop_prop[, id] <- 2 * pi * func_cdf_wc(theta[, id] - mu_prop, 0, rho_prop)
                ...
                # cc <- cos(theta_cop_prop[, id])
                # ss <- sin(theta_cop_prop[, id])
                ...
                # r_prop[, id] <- sqrt((rho2^2 - rho1^2) * u2 + rho1^2)

                # calcolo la propsota
                # if (any(!is.finite(A_vec)) || any(!is.finite(BA_vec)) || any(A_vec <= 0)) {
                #    mh_ratio <- -Inf
                # } else {
                # log_q_prop_r <- sum(log(r_prop[, id]) - 0.5 * A_vec * (r_prop[, id] - BA_vec)^2 - (log_Z_r(A_vec, BA_vec)))
                # adesso ricalcolo la proposta dell'accettato
                # cc <- cos(theta_cop[, id])
                # ss <- sin(theta_cop[, id])
                # A_vec <- prec_c * cc^2 + prec_s * ss^2
                # B_vec <- prec_c * cc * cond_mean_c_vec + prec_s * ss * cond_mean_s_vec
                # BA_vec <- B_vec / A_vec
                # log_q_mcmc_r <- sum(log(r_mcmc[, id]) - 0.5 * A_vec * (r_mcmc[, id] - BA_vec)^2 - (log_Z_r(A_vec, BA_vec)))

                # ! alcolo la nuova copula e osservazioni latenti
                # theta_cop_prop[, id] <- 2 * pi * func_cdf_wc(theta[, id] - mu_prop, 0, rho_prop)
                ...
                # )
                mh_ratio <- 0
                mh_ratio <- mh_ratio # + sum(log(r_prop[, id]) - log(r_mcmc[, id]))

                if (sum_iter > iter_ind) {
                    mh_ratio <- mh_ratio +
                        sum(dnorm(x_c_prop[, id], cond_mean_c_vec, sd_c, log = TRUE)) +
                        sum(dnorm(x_s_prop[, id], cond_mean_s_vec, sd_s, log = TRUE)) -
                        sum(dnorm(x_c_mcmc[, id], cond_mean_c_vec, sd_c, log = TRUE)) -
                        sum(dnorm(x_s_mcmc[, id], cond_mean_s_vec, sd_s, log = TRUE))
                }
                ...
                mh_ratio <- mh_ratio +
                    sum(func_logd_wc(theta[, id], mu_prop, rho_prop)) -
                    sum(func_logd_wc(theta[, id], mu_mcmc[id], rho_mcmc[id]))
                ...
                mh_ratio <- mh_ratio +
                    ...
                    dnorm(mu_mcmc[id], prior_mu_mean[id], sqrt(prior_mu_var[id]), log = TRUE)
                # prior rho
                rho_prop_app <- par_cop_prop[1]
                rho_mcmc_app <- par_cop_mcmc[[id]][1]
                mh_ratio <- mh_ratio +
                    (dbeta(rho_prop, prior_rho_a[id], prior_rho_b[id], log = TRUE) + rho_prop_app - 2 * log(1 + exp(rho_prop_app))) -
                    (dbeta(rho_mcmc[id], prior_rho_a[id], prior_rho_b[id], log = TRUE) + rho_mcmc_app - 2 * log(1 + exp(rho_mcmc_app)))

                # ! proposta r
                # mh_ratio <- mh_ratio + log_q_mcmc_r - log_q_prop_r
                # }
            ...
            if ((sum_iter < burnin)) {
                for (id in 1:d)
                {
                    gamma_adapt <- adapt_a / (adapt_b + sum_iter)

                    lambda_adapt[[id]] <- exp(log(lambda_adapt[[id]]) + gamma_adapt * (alpha_par_cop[[id]] - adapt_alpha_target))

                    mean_adapt[[id]] <- mean_adapt[[id]] + gamma_adapt * (par_cop_mcmc[[id]] - mean_adapt[[id]])


                    sigma_adapt[[id]] <- sigma_adapt[[id]] + gamma_adapt * ((par_cop_mcmc[[id]] - mean_adapt[[id]]) %*% t(par_cop_mcmc[[id]] - mean_adapt[[id]]) - sigma_adapt[[id]])
                    sigma_adapt[[id]] <- (sigma_adapt[[id]] + t(sigma_adapt[[id]])) / 2

                    eps_pd <- 1e-8
                    max_try <- 30
                    success <- FALSE

                    for (i_try in seq_len(max_try)) {
                        out <- tryCatch(
                            t(chol(lambda_adapt[[id]] * sigma_adapt[[id]] + diag(eps_pd, 2))),
                            error = function(e) NULL
                        )
                    ...
                    if (!success) {
                        stop("Cannot make multiplier_adapt_cor * sigma_parcor positive definite, even after adding diagonal jitter.")
                    }
                }
            }
